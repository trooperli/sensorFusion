...
The state we are tracking is [a, b, c, d] and the measurements from the camera are usually a series of points [(x0, y0), (x1, y1), ... (xN, yN)] in the image space. From the adasis map and ego vehicle motion, we could predict the state to the next timestamp. The details of the lane fusion algorithms need to be investigated further.
